...
==============================================================================
  ...
  Purpose : Debloat + performance tuning for 8 Roblox clones
...
local STATE_FILE     = SATURNITY_DIR.."/state.json"
...
local function shell(cmd)
    ...
    os.remove(tmp_out)
    os.remove(tmp_err)
    -- Lua 5.4 returns boolean + reason + code
...
local function log(level, msg)
    ...
    -- Log file (no color)
    log_raw(string.format("[%s] [%-5s] %s", ts, level, msg))
    -- Failure log
...
local function log_cmd_failure(action, cmd, code, err)
...
local function check_update()
    ...
    local out, err, code = shell("curl -fsSL --max-time 6 "..VERSION_URL)
...
local function phase_debloat()
    log("PHASE", "Phase 1: Debloat ("..#DEBLOAT_PACKAGES.." packages)")
    -- read existing disabled list to not double-record
    local existing = {}
    if file_exists(DISABLED_LIST) then
        for line in io.lines(DISABLED_LIST) do existing[trim(line)] = true end
    end
    -- safety: build clone lookup
    local protected = {}
    ...
    for _, pkg in ipairs(DEBLOAT_PACKAGES) do
        if protected[pkg] then
            log("WARN", "REFUSED to touch protected pkg: "..pkg)
        else
            -- check if installed
            local out = sush("pm list packages "..pkg)
            if not out:find(pkg, 1, true) then
                log("SKIP", pkg.." (not installed)")
            else
                -- check if already disabled
                local s_out = sush("pm list packages -d "..pkg)
                if s_out:find(pkg, 1, true) then
                    log("SKIP", pkg.." (already disabled)")
                    if not existing[pkg] then table.insert(newly_disabled, pkg) end
                else
                    ...
                    if code == 0 then
                        ...
                        log("FAIL", "Could not disable "..pkg)
                        log_cmd_failure("disable", cmd, code, err)
                    end
                end
            end
        end
    ...
    if #newly_disabled > 0 then
        ...
        log("INFO", "Recorded "..#newly_disabled.." new packages in disabled.list")
    end
end
...
local SETTINGS_TWEAKS = {
    -- Animations off (HUGE UI speed boost)
...
local function phase_settings()
    ...
    for _, t in ipairs(SETTINGS_TWEAKS) do
        ...
        if code == 0 then
            ...
            log_cmd_failure("settings", cmd, code, err)
        end
    end
end
...
local PERSIST_PROPS = {
    -- Logging buffer down (saves ~15 MB)
    ...
    -- Faster post-force-stop RAM release
    {key="persist.sys.purgeable_assets",      val="1"},
    -- LMK: clone-aware (kill heaviest hog, not random)
    {key="persist.sys.lmk.kill_heaviest_task",val="true"},
    {key="persist.sys.lmk.use_minfree_levels",val="true"},
}

-- Non-persist props go in /data/local.prop so they survive reboot
...
local function phase_props()
    ...
    for _, p in ipairs(PERSIST_PROPS) do
        ...
        if code == 0 then
            ...
            log_cmd_failure("setprop", cmd, code, err)
        end
    end
    -- /data/local.prop
    ...
    -- merge: keep non-saturnity lines, replace saturnity lines
    local kept = {}
    ...
    if code == 0 then
        ...
        log_cmd_failure("write prop", "cp /data/local.prop", code, err)
    end
end

-- ============================================================
-- PHASE 4 — VOLATILE TWEAKS (RESET AT BOOT — RE-RUN TO REAPPLY)
-- ============================================================

local function phase_volatile()
    log("PHASE", "Phase 4: Volatile tweaks (re-run after reboot)")
    ...
    if cpu_count > 0 then
        ...
        for i = 0, cpu_count - 1 do
            ...
            local _, _, code = sush("[ -w "..path.." ] && echo performance > "..path)
            if code == 0 then set_ok = set_ok + 1 end
        end
        log("OK", string.format("CPU governor set to performance on %d/%d cores", set_ok, cpu_count))
    else
        log("WARN", "Could not enumerate CPU cores")
    end
    -- Kernel VM tweaks
    local vm = {
    ...
    for _, e in ipairs(vm) do
        local _, err, code = sush("echo "..e[2].." > "..e[1])
        if code == 0 then
            log("OK", e[1].." = "..e[2])
        else
            log("WARN", "Could not write "..e[1])
        end
    end
    -- Drop caches (one-time RAM free)
    sush("sync && echo 3 > /proc/sys/vm/drop_caches")
    log("OK", "Caches dropped (sync + drop_caches=3)")
    -- Network tuning
    local net = {
    ...
    for _, n in ipairs(net) do
        local _, _, code = sush("sysctl -w "..n)
        if code == 0 then
            log("OK", "sysctl "..n)
        else
            log("WARN", "sysctl failed: "..n)
        end
    end
end

-- ============================================================
-- PHASE 5 — ROBLOX CLONE PROTECTION
-- ============================================================

local function phase_clone_protection()
    log("PHASE", "Phase 5: Roblox clone protection")
    for _, pkg in ipairs(ROBLOX_CLONES) do
        -- verify installed
        local out = sush("pm list packages "..pkg)
        if not out:find(pkg, 1, true) then
            log("WARN", pkg.." not installed — skipped")
        else
            -- allow background
            local _, err1, code1 = sush("appops set "..pkg.." RUN_IN_BACKGROUND allow")
            if code1 == 0 then
                log("OK", pkg.." background allowed")
            else
                log("WARN", "appops RUN_IN_BACKGROUND on "..pkg)
            end
            -- mark active (not idle)
            local _, _, code2 = sush("am set-inactive "..pkg.." false")
            if code2 == 0 then
                log("OK", pkg.." marked active")
            end
...
local function print_summary(ram_before, ram_after)
    ...
    print(C.bold..C.pink.."  🪐 SATURNITY OPTIMIZER — SUMMARY"..C.reset)
    print(C.saturn..string.rep("═", 56)..C.reset)
    ...
    print(C.saturn..string.rep("─", 56)..C.reset)
    print(string.format("  RAM before : %s%.0f MB%s available", C.cyan, ram_before, C.reset))
    print(string.format("  RAM after  : %s%.0f MB%s available", C.cyan, ram_after,  C.reset))
...
local function main()
    -- detect re-launch after self-update to skip loop
